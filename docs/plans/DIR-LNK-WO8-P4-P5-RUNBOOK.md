...
| g | **BACKFILL** | current data (post any reverts) | Run `resyncAllLinked` via `POST /ajax/master/resync-all.cfm` (admin) | response `{scanned:16, synced:0, skipped:16, failed:0, fieldsWritten:0}` (no-op against current data); counts visible | none |
...
| i | **ADMIN GATE (N-3)** | — | `POST /ajax/master/resync-all.cfm` as a non-admin, then as an admin | non-admin → 403 `{success:false, "Admin access required."}`; admin (`taousers.userRole` IN Admin/Administrator) → runs. (Confirms the gate is neither permanently-403 nor open) | none |
