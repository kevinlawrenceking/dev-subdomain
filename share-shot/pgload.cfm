...
<!--- Load the UserService --->
 ...
 <cfquery name="userShareData"  datasource="abo">
            SELECT
                u.userid,
                u.recordname,
                u.userFirstName,
            ...
            WHERE LEFT(u.passwordhash, 10) = <cfqueryparam value="#uid#" cfsqltype="CF_SQL_CHAR">
        </cfquery>
...
<cfif userShareData.recordCount EQ 0>
    <cfoutput>Not found!</cfoutput>
    <cfabort>
<cfelse>
    <!--- Map user data to local variables --->
    <cfset userid = userShareData.userId />
    <cfset userContactid = userShareData.userContactID />
    <cfset userCalStarttime = userShareData.calstarttime />
    <cfset userCalendtime = userShareData.calendtime />
    <cfset avatarname = userShareData.avatarname />
    <cfset isbetatester = userShareData.isBetaTester />
    <cfset defrows = userShareData.defRows + 1 />
    <cfset defcountry = userShareData.defCountry />
    <cfset defState = userShareData.defState />
    <cfset tzid = userShareData.tzid />
    <cfset customerid = userShareData.customerid />
    <cfset userFirstName = userShareData.userFirstName />
    <cfset userLastName = userShareData.userLastName />
    <cfset userEmail = userShareData.userEmail />
    <cfset userRole = userShareData.userRole />
