...
SET ticketResponse = CONCAT(
    COALESCE(ticketResponse, ''),
    '\n[IMPLEMENTED 2026-03-20] Root cause: The form submit handler in note-update-event.cfm and note-update-aud.cfm used $(\"#snow-editor\").html() to capture the Quill editor content. This captured the ENTIRE Quill container HTML including .ql-editor and .ql-clipboard wrapper divs, not just the content. On each save, the content got wrapped in extra divs, causing progressive corruption. Fix: Changed to $(\"#snow-editor .ql-editor\").html() to capture only the actual content. Also added sync of the noteDetails textarea from Quill text for plain-text consistency. Files changed: include/note-update-event.cfm, include/note-update-aud.cfm, include/note-add-event.cfm'
),
    ticketStatus = 'Implemented',
...
SET ticketResponse = CONCAT(
    COALESCE(ticketResponse, ''),
    '\n[IMPLEMENTED 2026-03-20] Root cause: The SAME button click handler in remoteaudupdateform.cfm used unescaped CF values in JavaScript string literals. If any location field (eventLocation, address, city, etc.) contained quotes, backslashes, or newlines, the entire script block failed silently at parse time, making the button appear non-functional. Fix: (1) Wrapped all CF values with jsStringFormat() to properly escape for JS. (2) Added null-safety checks (if el) on all getElementById calls. (3) Added setTimeout for region_id to allow filterRegions() to refresh dropdown options before setting the value. Files changed: include/remoteaudupdateform.cfm'
),
    ticketStatus = 'Implemented',
...
SET ticketResponse = CONCAT(
    COALESCE(ticketResponse, ''),
    '\n[IMPLEMENTED 2026-03-20] Fix was already applied in a prior session. Root cause: Safari/older browsers did not auto-correct EXIF Orientation tags on uploaded JPG images, causing portrait photos to appear squished. Fix: Added CSS rules in tao-components.css: (1) .tao-avatar class has image-orientation:from-image and object-fit:cover with explicit width/height, (2) Legacy avatar classes (.current-avatar, .birthday-avatar, etc.) also get image-orientation:from-image. Avatar img tags in contact_view.cfm use the tao-avatar class. Files changed: app/assets/css/tao-components.css, include/contact_view.cfm'
),
    ticketStatus = 'Implemented',
...
SET ticketResponse = CONCAT(
    COALESCE(ticketResponse, ''),
    '\n[IMPLEMENTED 2026-03-20] Fix was already applied in a prior session. Root cause: The Quill.js rich text editor did not strip inline color/background-color styles when pasting content. Text with white font color from dark-background websites became invisible on TAO''s white background. Fix: Added a clipboard matcher in form-quill.js (lines 38-49) that removes color and background attributes from pasted content via quill.clipboard.addMatcher(). The toolbar still has the color picker for intentional color changes. Files changed: app/assets/js/form-quill.js'
),
    ticketStatus = 'Implemented',
