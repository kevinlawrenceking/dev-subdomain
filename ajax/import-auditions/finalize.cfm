<cfsilent>
<!---
    Audition Import - Finalize Endpoint
    POST /ajax/import-auditions/finalize.cfm

    Creates new auditions from approved rows (create-only mode).
...
<cftry>
    ...
    <cflog file="import_auditions" text="[finalize] START userid=#variables.userid#">
    ...
    <cfif variables.jobId lte 0>
        <cfset returnError("MISSING_JOB_ID", "job_id is required", 400)>
    </cfif>
    ...
    <cflog file="import_auditions" text="[finalize] CALLING_SERVICE userid=#variables.userid# job_id=#variables.jobId# status=#variables.job.status#">
    <cfset variables.finalizeResult = variables.auditionService.finalizeJob(variables.jobId, variables.userid)>

    <cfif variables.finalizeResult.success>
        <cflog file="import_auditions" text="[finalize] SUCCESS userid=#variables.userid# job_id=#variables.jobId# elapsed_ms=#getTickCount() - variables.startTick#">
    ...
    <cfcatch type="any">
        <cflog file="import_auditions" text="[finalize] ERROR userid=#variables.userid# job_id=#variables.jobId# message=#cfcatch.message# detail=#cfcatch.detail#">
