<cfsilent>
<!---
    Audition Import - Rows List Endpoint
    GET /ajax/import-auditions/rows.cfm

    Returns paginated list of rows for an audition import job with summary counts.
    ...
    - search (optional): Search across contact_name, project_name, role_name, casting_director
...
<cftry>
    ...
    <cflog file="import_auditions" text="[rows] START userid=#variables.userid#">
    ...
    <cfif variables.statsOnly>
        <cfset variables.stats = variables.auditionService.getJobStats(variables.jobId)>
        <cfset arrayAppend(variables.debug, "stats_fetched")>
        <cfset arrayAppend(variables.debug, "done")>
        <cflog file="import_auditions" text="[rows] STATS_ONLY userid=#variables.userid# job_id=#variables.jobId# elapsed_ms=#getTickCount() - variables.startTick#">
    ...
    <cfelse>
        ...
        <cfset variables.rowsResult = variables.auditionService.getRows(
        ...
        <cfset arrayAppend(variables.debug, "done")>
        <cflog file="import_auditions" text="[rows] SUCCESS userid=#variables.userid# job_id=#variables.jobId# filter=#variables.statusFilter# page=#variables.page# elapsed_ms=#getTickCount() - variables.startTick#">
    ...
    <cfcatch type="any">
        ...
        <!--- Also log to import_auditions log file --->
        <cflog file="import_auditions" text="rows.cfm ERROR job_id=#variables.jobId# user=#variables.userid# err=#cfcatch.message# detail=#cfcatch.detail# type=#cfcatch.type# tagcontext=#cfcatch.tagcontext[1].template#:#cfcatch.tagcontext[1].line#">
