...
<!---
    ...
    - Example: ["start","auth_ok","file_validated","file_uploaded","hash_computed","dupe_checked","job_created","done"]
--->
...
<cflog file="importv3" text="[upload] START">
<cftry>
    ...
    <cftry>
        ...
        <cfcatch type="any">
            ...
            <cflog file="importv3" text="[upload] ERROR upload failed userid=#variables.userid# message=#cfcatch.message#">
            <cfset variables.response.data.debug = variables.debug>
            <cfcontent type="application/json" reset="true"><cfoutput>#serializeJSON(variables.response)#</cfoutput><cfabort>
        </cfcatch>
    </cftry>
    <cfset arrayAppend(variables.debug, "file_uploaded")>
    <cflog file="importv3" text="[upload] FILE_UPLOADED userid=#variables.userid# file=#cffile.clientfile#">
    ...
    <cfif not listFindNoCase("csv,xls,xlsx,vcf", variables.fileType)>
        <!--- Delete invalid file --->
        <cffile action="delete" file="#variables.uploadedPath#">
        <cfset variables.response.code = "INVALID_FILE_TYPE">
        <cfset variables.response.message = "Invalid file type. Allowed: CSV, XLS, XLSX, VCF">
        <cflog file="importv3" text="[upload] ERROR invalid file type userid=#variables.userid# type=#variables.fileType#">
        <cfset variables.response.data.debug = variables.debug>
        <cfcontent type="application/json" reset="true"><cfoutput>#serializeJSON(variables.response)#</cfoutput><cfabort>
    </cfif>
    ...
    <cfif variables.fileSize gt 52428800>
        ...
        <cflog file="importv3" text="[upload] ERROR file too large userid=#variables.userid# size=#variables.fileSize#">
        <cfset variables.response.data.debug = variables.debug>
        <cfcontent type="application/json" reset="true"><cfoutput>#serializeJSON(variables.response)#</cfoutput><cfabort>
    </cfif>
    ...
    <cfif variables.qExisting.recordCount gt 0>
        ...
        <cflog file="importv3" text="[upload] DUPLICATE userid=#variables.userid# existing_job_id=#variables.qExisting.job_id# hash=#variables.fileHash#">
    ...
    <cflog file="importv3" text="[upload] SUCCESS userid=#variables.userid# job_id=#variables.newJobId# hash=#variables.fileHash# elapsed_ms=#getTickCount() - variables.startTick#">
    ...
    <cfcatch type="any">
        <cflog file="importv3" text="[upload] ERROR userid=#variables.userid# message=#cfcatch.message# detail=#cfcatch.detail#">
