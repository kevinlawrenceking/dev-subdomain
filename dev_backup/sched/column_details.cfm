<cfquery result="result" name="tables" datasource="abod">
    ...
    WHERE t.qry_table IS NOT NULL 
        AND t.qry_table <> '' 
        AND t.qry_view IS NULL 
    
</cfquery>

<cfloop query="tables">

    <cfset new_table = tables.qry_table />
    ...
    <cfquery result="result" name="getColumnDetails" datasource="abod">
        ...
        WHERE 
            ...
            AND c.table_name = <cfqueryparam value="#new_table#" cfsqltype="cf_sql_varchar">   
        ORDER BY 
            c.ORDINAL_POSITION;
    </cfquery>
    ...
    <cfsavecontent variable="generatedColumnData"><cfoutput>
        <h3>Column Details for table: '#new_table#'</h3>
        <table border="1">
            <tr>
                <th>Column Name</th>
                <th>Data Type</th>
