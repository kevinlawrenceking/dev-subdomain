...
<cfif NOT reFindNoCase("^[a-z_][a-z0-9_]{0,63}$", trim(rpg_comptable))>
    <cflog file="remoteUpdateFormUpdate" text="BLOCKED: invalid table name rpg_comptable='#htmlEditFormat(rpg_comptable)#'" />
    <cfabort>
</cfif>
<cfif NOT reFindNoCase("^[a-z_][a-z0-9_]{0,63}$", trim(fid))>
    <cflog file="remoteUpdateFormUpdate" text="BLOCKED: invalid column name fid='#htmlEditFormat(fid)#'" />
    <cfabort>
</cfif>
    ...
    <cfif #rpgupdate.fname# is not "#fid#">
        <cfoutput>
            ...
            <cfif NOT reFindNoCase("^[a-z_][a-z0-9_]{0,63}$", trim(fname))>
                <cflog file="remoteUpdateFormUpdate" text="BLOCKED: invalid field name fname='#htmlEditFormat(fname)#'" />
                <cfcontinue>
            </cfif>
