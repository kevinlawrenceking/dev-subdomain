<!--- 
    PURPOSE: Modern contact avatar upload and cropping interface
    AUTHOR: Kevin King
    DATE: 2025-07-27
...
$(document).ready(function() {
    ...
    setTimeout(function() {
        console.log('DOM ready, initializing contact avatar upload app...');
        initializeContactUploadApp();
...
<!-- Modern Contact Avatar Upload Interface -->
<!-- Header Section -->
<div class="text-center mb-4 fade-in">
    <p class="text-muted fs-5 mb-0">
        <cfoutput>Upload a new avatar for <strong>#subtitle#</strong></cfoutput>
    </p>
    <div class="mt-3">
...
<div class="card upload-card mt-4 success-animation" id="success-section" style="display: none;">
    <div class="card-body p-4 text-center">
        ...
        <p class="text-muted mb-4">The contact's avatar has been updated and is now visible across the platform.</p>
        <div class="mb-4">
        ...
        <div class="d-flex justify-content-center gap-3">
            <cfoutput>
                <a href="#cookie.return_url#" class="btn btn-primary-modern">
                    <i class="fe-arrow-right me-2"></i>Continue to Contact
                </a>
            </cfoutput>
            <button type="button" class="btn btn-outline-secondary btn-modern" onclick="window.location.reload();">
                <i class="fe-refresh-cw me-2"></i>Upload Another
            </button>
        </div>
        <div class="mt-3">
            <small class="text-muted">
...
<!-- Enhanced JavaScript with Modern Features for Contact Avatars -->
<script>
// Main initialization function for contact avatar upload
function initializeContactUploadApp() {
    // Prevent multiple initializations
    if (window.contactUploadInterfaceInitialized) return;
    window.contactUploadInterfaceInitialized = true;
    
    console.log('Initializing contact avatar upload interface...');
    ...
    function initializeCroppie() {
        console.log('Initializing Contact Avatar Croppie...');
        ...
        if (!useCroppie) {
            ...
            console.log('Loading current contact avatar:', currentAvatarUrl);
            ...
            testImg.onload = function() {
                console.log('Current contact avatar loaded successfully');
                // Remove the overlay text since image loaded
                $('#basic-preview span').hide();
            };
            testImg.onerror = function() {
                console.warn('Current contact avatar failed to load');
                $('#basic-preview').css({
        ...
        try {
            console.log('Creating Contact Avatar Croppie instance...');
            ...
            console.log('Contact Avatar Croppie instance created successfully');
            
        } catch (error) {
            console.error('Error initializing Contact Avatar Croppie:', error);
            ...
            const currentAvatarUrl = '<cfoutput>#image_url#</cfoutput>?ver=<cfoutput>#rand()#</cfoutput>';
            console.log('Fallback: Loading current contact avatar:', currentAvatarUrl);
            ...
            const testImg = new Image();
            testImg.onload = function() {
                console.log('Fallback: Current contact avatar loaded successfully');
                $('#fallback-preview span').hide();
            };
            testImg.onerror = function() {
                console.warn('Fallback: Current contact avatar failed to load');
        ...
        reader.onload = function (e) {
            ...
            if (useCroppie) {
                try {
                    ...
                    console.log('Binding new image to Contact Avatar Croppie...');
                    $uploadCrop.bind({ url: e.target.result })
                        .then(function() {
                            console.log('New contact avatar image loaded for cropping successfully');
                        })
                        .catch(function(error) {
                            console.error('Error binding image to Contact Avatar Croppie:', error);
                            showErrorMessage('Failed to load image for cropping. Please try again.');
                        });
                } catch (error) {
                    console.error('Error initializing Contact Avatar Croppie for new image:', error);
                ...
                $('#upload-input').html(`
                    ...
                    <p class="mt-3 text-muted">Preview of your new contact avatar</p>
                `);
            }
        };
    ...
    $('#upload-zone').on('click', function(e) {
        ...
        if (e.target.id !== 'upload') {
            e.preventDefault();
            e.stopPropagation();
            console.log('Contact avatar upload zone clicked, opening file browser...');
    ...
    $('#upload').on('change', function(e) {
        console.log('Contact avatar file input changed...');
    ...
    // Save cropped image - using contact-specific endpoint
    $('#crop-save-button').on('click', function() {
        ...
        if (useCroppie && $uploadCrop) {
            ...
            try {
                console.log('Getting cropped contact avatar result using direct constructor API...');
                ...
                if (croppieResult && typeof croppieResult.then === 'function') {
                    console.log('Processing cropped contact avatar result promise...');
                    croppieResult.then(function(resp) {
                        console.log('Cropped contact avatar result received:', resp ? 'success' : 'empty');
                        saveContactImageData(resp, $button, originalText);
                    }).catch(function(error) {
                        console.error('Error getting cropped contact avatar result:', error);
                        showErrorMessage('Failed to process cropped image. Please try again.');
                        $button.html(originalText).prop('disabled', false);
                    });
                } else {
                    console.error('Expected promise from Contact Avatar Croppie result, got:', typeof croppieResult);
                    showErrorMessage('Failed to process cropped image. Please try again.');
                    $button.html(originalText).prop('disabled', false);
                }
            } catch (error) {
                console.error('Error in contact avatar crop save process:', error);
            ...
            saveContactImageData(selectedImageData, $button, originalText);
        } else {
    ...
    // Function to save contact image data - using contact-specific endpoint
    function saveContactImageData(imageData, $button, originalText) {
        $.ajax({
            ...
            success: function(data) {
                console.log('Contact avatar upload successful:', data);
            ...
            error: function(xhr, status, error) {
                console.error('Contact avatar Ajax error:', xhr, status, error);
                $button.html(originalText).prop('disabled', false);
                
                if (status === 'timeout') {
                    showErrorMessage('Upload timed out. Please try again with a smaller image.');
                } else {
                    showErrorMessage('Failed to save contact avatar. Please try again.');
                }
            }
        });
    }
    ...
    console.log('About to initialize Contact Avatar Croppie...');
    console.log('Current useCroppie value:', useCroppie);
    
    setTimeout(function() {
        console.log('Running delayed Contact Avatar Croppie initialization...');
        initializeCroppie();
        
        // Log the final state
        console.log('Contact avatar upload interface fully initialized');
        ...
        window.debugContactCroppie = function() {
            console.log('=== CONTACT AVATAR CROPPIE DEBUG INFO ===');
            ...
            console.log('Contact upload interface initialized:', window.contactUploadInterfaceInitialized);
            console.log('Current useCroppie value:', useCroppie);
            
            // Test current avatar URL
            const avatarUrl = '<cfoutput>#image_url#</cfoutput>?ver=<cfoutput>#rand()#</cfoutput>';
            console.log('Current contact avatar URL:', avatarUrl);
            
            // Test image loading
            const testImg = new Image();
            testImg.onload = () => console.log('✓ Contact avatar image loads successfully');
            testImg.onerror = () => console.log('✗ Contact avatar image failed to load');
            ...
            if (typeof Croppie === 'function') {
                ...
                try {
                    ...
                    console.log('✓ Contact Avatar Croppie direct constructor test passed');
                    testCroppie.destroy();
                    document.body.removeChild(testEl);
                } catch (e) {
                    console.log('✗ Contact Avatar Croppie direct constructor test failed:', e.message);
                }
            } else {
                console.log('✗ Croppie global constructor not available');
            }
            
            console.log('=== END CONTACT AVATAR DEBUG INFO ===');
        };
        
        // Auto-run debug after a delay
        setTimeout(function() {
            console.log('Running automatic Contact Avatar Croppie debug check...');
            window.debugContactCroppie();
        }, 2000);
    }, 100);
}
</script>